...
namespace Templar.Helpers
{
    ...
    public class WindowSettings
    {
        ...
        public void Record(Form form, params SplitContainer[] splitters)
        {
            ...
            if (form == null)
            {
                shouldRecordSplitters = (
                    from splitter in splitters
                    where splitter != null
                    let findForm = splitter.FindForm()
                    where findForm != null
                    select findForm.WindowState into formState
                    select formState != FormWindowState.Minimized
                ).FirstOrDefault();
            }
            else
            {
                switch (form.WindowState)
                {
                    ...
                    case FormWindowState.Normal:
                        shouldRecordSplitters = RecordWindowPosition(form.Bounds);
                        break;
                    default:
        ...
        private void RestoreSplitters(SplitContainer[] splitters)
        {
            for (
                var i = 0;
                i < splitters.Length && SplitterDistances != null && i < SplitterDistances.Length;
                i++
            )
            {
        ...
        {
            foreach (
                var workingArea in Screen.AllScreens.Select(screen => new Rectangle(
