...
namespace Templar.Helpers
{
    public class PriorityTreeState
    {
            ...
            if (!Variables.AlteredSettings)
            {
                ...
                foreach (var bagItem in StyxWoW.Me.BagItems)
                {
                    ...
                    CustomLog.Diagnostic(
                        "Inventory item added. Name: {0}, Entry: {1}, StackCount: {2}",
                        bagItem.Name,
                        bagItem.Entry,
                        bagItem.StackCount
                    );
        ...
        private static void LogSettings()
        {
            ...
            CustomLog.Diagnostic(
                string.Format("Ignore Elites: {0}", GeneralSettings.Instance.IgnoreElites)
            );
            CustomLog.Diagnostic(
                string.Format("Loot Mobs: {0}", GeneralSettings.Instance.LootMobs)
            );
            CustomLog.Diagnostic(
                string.Format("Time Between Loots: {0}", GeneralSettings.Instance.TimeBetweenLoots)
            );
            CustomLog.Diagnostic(
                string.Format("Skin Mobs: {0}", GeneralSettings.Instance.SkinMobs)
            );
            CustomLog.Diagnostic(
                string.Format("Ninja Skin: {0}", GeneralSettings.Instance.NinjaSkin)
            );
            CustomLog.Diagnostic(
                string.Format("Time Between Skins: {0}", GeneralSettings.Instance.TimeBetweenSkins)
            );
            CustomLog.Diagnostic(
                string.Format("Multi Pull: {0}", GeneralSettings.Instance.MultiPull)
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Multi Pull Mob Threshold: {0}%",
                    GeneralSettings.Instance.MobThreshold
                )
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Multi Pull Health Threshold: {0}",
                    GeneralSettings.Instance.HealthPercentThreshold
                )
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Start Location Max Distance: {0}",
            ...
            foreach (var whitelistedUnit in WhitelistSettings.Instance.WhitelistedUnits)
            {
                CustomLog.Diagnostic(
                    string.Format(
                        "Name: {0} Entry: {1}",
                        whitelistedUnit.Name,
                        whitelistedUnit.Entry
                    )
                );
            }
            CustomLog.Diagnostic(
                string.Format(
                    "Attack Only Whitelist: {0}",
            ...
            foreach (var blacklistedUnit in BlacklistSettings.Instance.BlacklistedUnits)
            {
                CustomLog.Diagnostic(
                    string.Format(
                        "Name: {0} Entry: {1}",
                        blacklistedUnit.Name,
                        blacklistedUnit.Entry
                    )
                );
            }
            ...
            {
                CustomLog.Diagnostic(
                    string.Format("Name: {0} Entry: {1}", protectedItem.Name, protectedItem.Entry)
                );
            }
            CustomLog.Diagnostic("**** MAIL TAB ****");
            CustomLog.Diagnostic(string.Format("Mailing: {0}", MailSettings.Instance.Mail));
            CustomLog.Diagnostic(
                string.Format("Mail Whites: {0}", MailSettings.Instance.MailWhites)
            );
            CustomLog.Diagnostic(
                string.Format("Mail Greens: {0}", MailSettings.Instance.MailGreens)
            );
            CustomLog.Diagnostic(string.Format("Mail Blues: {0}", MailSettings.Instance.MailBlues));
            CustomLog.Diagnostic(
                string.Format("Mail Purples: {0}", MailSettings.Instance.MailPurples)
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Recipient: {0}",
            ...
            CustomLog.Diagnostic(
                string.Format("Sell Grays: {0}", VendorSettings.Instance.SellGrays)
            );
            CustomLog.Diagnostic(
                string.Format("Sell Whites: {0}", VendorSettings.Instance.SellWhites)
            );
            CustomLog.Diagnostic(
                string.Format("Sell Greens: {0}", VendorSettings.Instance.SellGreens)
            );
            CustomLog.Diagnostic(
                string.Format("Sell Blues: {0}", VendorSettings.Instance.SellBlues)
            );
            CustomLog.Diagnostic(
                string.Format("Sell Purples: {0}", VendorSettings.Instance.SellPurples)
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Repair at: {0}%",
            ...
            CustomLog.Diagnostic(
                string.Format("Disenchanting: {0}", DisenchantSettings.Instance.Disenchant)
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Disenchant Only New Items: {0}",
            ...
            CustomLog.Diagnostic(
                string.Format("Disenchant Blues: {0}", DisenchantSettings.Instance.DisenchantBlues)
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Disenchant Purples: {0}",
                    DisenchantSettings.Instance.DisenchantPurples
                )
            );
            CustomLog.Diagnostic(
                string.Format(
                    "Time Between Disenchants: {0}",
